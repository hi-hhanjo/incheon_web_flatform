...
- **목적**: `lib/api/matches.ts`·`standings.ts`·`opponentScouting.ts`가 지금 `data/*.json` mock으로 흉내 내는 데이터를, SQLite 테이블로 옮기기 위한 스키마를 정의합니다.
...
PRD.md F-16·F-17에 따르면 순위표·지난 경기·최근 5경기는 무료 API로 실시간 연동이 불가능해, **초안을 만들고 사람이 검수 후 반영**하는 방식으로 운영합니다. 초안 생성 수단은 2026-07-16부로 공개 순위 API 크롤링(다음 스포츠, `scraper/`, CRAWLER_SPEC.md)으로 전환했습니다 — 크롤러가 이 스키마와 일치하는 `data/standings.json`을 만들면 `scripts/seed-db.mjs`가 아래 테이블로 적재합니다(순위표 구현 완료, 자동 스케줄링 자체는 미구현).
