...
scraper/
  ...
  kleague/
    ...
    schedule.py                 # parse_incheon_matches / parse_recent_form: 경기 JSON → 앱 스키마
...
- **상대 폼(F-18)**: 다음 상대가 실시간으로 바뀌므로 `export_opponent_forms.py`가 12팀 전체의
  최근 5경기(`parse_recent_form`)를 미리 크롤 → `data/opponent-forms.json`(앱 팀명 키). 화면은
  그때의 상대명으로 조회한다. 주요 선수·부상·라인업은 선수 개인정보 리스크로 mock 유지(F-18).
...
- **순위 이력 활용**: `standings-history/*.json`으로 "지난주 대비 순위 변동(▲▼)" 표시(미구현).
