...
- **문서 버전**: v2.8
- **작성일**: 2026-07-16 (v2.8 갱신: 2026-07-19)
...
```
scraper/
  requirements.txt              # requests, beautifulsoup4(스카우팅 부상 파싱용)
  kleague/
    client.py                   # fetch_rank() / fetch_schedule() / fetch_person_rank(): 다음 공개 API GET
    codes.py                    # 다음·Transfermarkt 팀명 → 앱 표기 정규화(단일 매핑)
  ...
  export_scouting.py            # 12팀 주요 선수(다음)·부상(Transfermarkt) → opponent-scouting.json
```
...
- **상대 폼(F-18)**: 다음 상대가 실시간으로 바뀌므로 `export_opponent_forms.py`가 12팀 전체의
  **시즌 경기 전체**(`parse_team_form`, 최신순)를 미리 크롤 → `data/opponent-forms.json`(앱 팀명
  ...
  주요 선수·부상은 `export_scouting.py`가 실데이터로 수집한다(3.10절). 라인업(probableLineup)만
  아직 소스가 없어 빈 목록이다.
...
| 나무위키 | 다수 | △ | △ | **탈락** — 아래 참고 |
| [아누즈 유튜브 채널](https://www.youtube.com/channel/UCIEPr-kv6_nCdQ1-ueuypZQ) | 신곡 | ✅ 설명란에 전문(팬 채널) | ✅ | **채택** — **사람이 고른 URL만**(3.9절) |
| YouTube Data API | - | ❌ 설명란 의존 | ✅ | **탈락** — API 키·시크릿 의존이 되살아남(3.6.1의 전환 취지에 역행) |
...
| **가사가 있는데** 출처가 없는 곡 | **삭제** | 출처를 못 밝히는 가사라 PRD 9장을 만족할 수 없다 |
| **가사가 빈** 곡 | **남긴다** *(v2.6 정정)* | 밝힐 가사가 없으니 '출처 없음'이 결함이 아니다 — 영상만 있는 곡(3.9절 아누즈 신곡)이 여기 해당한다. 영상은 임베드라 애초에 출처 표기 대상이 아니다 |

> ⚠️ **v2.6 정정**: 원래 규칙은 "`source`가 없으면 삭제"였다. PRD 9장의 의도는 **가사** 출처인데
> 구현이 `"source" not in song`만 봐서, **가사가 빈 곡까지 지웠다**. 3.9절로 영상만 있는 곡
> (가사 없음·출처 없음)이 들어오면서 실제 사고가 됐다 — `reconcile_songs.py`가 방금 승격한
> 아누즈 9곡을 전부 삭제 대상으로 잡았다(`--dry-run`으로 발견). 조건에 `song["lyrics"]`를 더해
> **가사가 있는 곡에만** 삭제 규칙을 적용한다.
...
- 새 필드를 만들지 않고 **기존 스키마로** 표현한다: `category`(`"선수 응원가"`/`"팀 응원가"`,
  PRD F-01 목록 분류 정렬이 쓰는 분류명 그대로) + `tags`의 `"미사용"`.
...
- **선수 응원가 제목은 `OOO 콜`로 통일한다** *(v2.6 확정)*. 목록에서 선수 응원가임이 한눈에 보이고,
  소스마다 갈리던 표기("무고사" vs "무고사 콜")가 앱에서는 하나로 정리된다.
  - ⚠️ **크롤 소스는 여전히 맨 이름으로 부른다**(인천네이션: `"무고사"`·`"전재호"`). 그래서
    `merge.normalize_title()`이 **'콜' 접미사를 떼고** 대조한다 — 안 그러면 ① `reconcile`이
    제목을 못 찾아 **가사 갱신을 조용히 멈추고**, ② 영상이 없는 곡(`전재호 콜`)은 제목·영상
    양쪽 다 안 걸려 크롤이 **중복 곡으로 다시 추가**한다. 실제로 둘 다 재현했다.
  - 새 선수 응원가를 추가할 때도 이 형식을 지킬 것.
...
**현재 상태(2026-07-17, v2.6)**: 아누즈 신곡 2곡이 더해져 선수 응원가 **8곡** — **현역 2곡**
(무고사 콜·이명주 콜) · **미사용 6곡**(김도혁·남준재·부노자·임중용·전재호·김동민 콜). 제목은
전부 `OOO 콜` 형식이다. 김동민은 구단 공식 명단 50명에 없어 미사용으로 판정됐다(명단 파싱은
정상 — 팀명 오염 없이 이명주·무고사가 잡힌다).
...
## 3.9 아누즈 채널 신곡 반영 (v2.6 신설)

### 3.9.1 문제 — 신곡이 들어올 통로가 없다

`songs.json`의 영상은 **대부분 이미 아누즈 채널 것**이다. 다만 인천네이션 게시글(3.7.1)을
**거쳐서** 들어왔다 — 그 글은 2016~2019년 링크 모음이고 갱신되지 않는다. 그래서 이후 나온 신곡이
반영되지 않는다(2026-07-17 확인: 이명주 콜·김동민 콜·일기·Glory Incheon·Walk Along·알레 인천·
300만의 목소리·가자 아시아로·인천 만세 등). 공식 홈페이지는 4곡뿐이라 이 구멍을 못 메운다.

### 3.9.2 왜 채널을 자동 수집하지 않나 (중요)

**`youtube.com/robots.txt`가 채널 전량 조회 경로를 모두 Disallow한다** (2026-07-17 확인):

```
User-agent: *
Disallow: /feeds/videos.xml   ← 채널 RSS (최신 15개)
Disallow: /youtubei/          ← 채널 페이지 '더보기' 페이지네이션
```

3.7.1이 **나무위키를 탈락시킨 기준과 같다** — 봇 차단·크롤링 거부를 우회하지 않는다. 채널
RSS는 키가 필요 없어 매력적으로 보이지만 바로 이 Disallow 목록에 있다. `/videos` 페이지의
첫 화면(`ytInitialData`)만은 긁히지만, 그 이상은 `/youtubei/`를 거쳐야 해 전량 수집이 불가능하고
비공식 내부 JSON이라 깨지기도 쉽다.

**YouTube Data API도 탈락**시켰다. 허가된 경로지만 API 키 + Vercel/Actions 시크릿 의존이
되살아난다 — 3.6.1에서 애써 없앤 `.env` 요구를 되돌리는 셈이라, 신곡 몇 곡을 위해 치를 값이 아니다.

### 3.9.3 채택 — 사람이 고른 URL만 oembed + /watch로 조회

`https://www.youtube.com/oembed`(제목)와 `/watch`(설명란)는 **모두 Disallow 목록에 없다**.
그래서 채널을 훑지 않고, **사람이 고른 영상 URL만** 조회한다.

이 분업은 검수 큐 모델(3.7)과 자연스럽게 맞는다 — "이 영상이 신규 응원가인가", "공식인가 현장인가"는
어차피 사람 판단이고, 응원가는 1년에 몇 곡 수준이라(3.7.3 '자동화 제외') 자동화 이득이 작다.

```
scraper/chants/youtube.py            # oembed(제목) + /watch(설명란=가사) 조회 → 후보 만들기
scraper/add_youtube_chants.py        # URL들 → data/songs-candidates.json (검수 큐)
scraper/backfill_youtube_lyrics.py   # 이미 등록된 '가사 빈 곡'을 설명란으로 채움(3.9.5)
```
```bash
cd scraper
# 아누즈 채널에서 신곡 영상 URL을 골라 넘긴다 (인자 또는 표준입력)
python add_youtube_chants.py https://youtu.be/XXXXXXXXXXX https://youtu.be/YYYYYYYYYYY
python promote_candidates.py --dry-run    # 이후는 기존 흐름과 동일(3.7.3)
```

- **가사는 영상 설명란에서 온다** *(v2.7)*. 아누즈는 설명란에 가사 전문을 적어두고, 상당수는
  출처까지 밝혀둔다("출처 : 파랑검정 인스타그램" — 파랑검정은 인천 서포터즈). 채널이 스스로 공개한
  텍스트라 PRD 9장의 '출처가 확인된 자료'에 해당한다. `_review.lyricsConfidence`는 `"community"`
  (팬 채널이므로 구단 공식보다 아래). 설명란이 없으면 `""`로 두고 `"none"`이 된다.
  - **`출처 :` 꼬리표는 떼어낸다** — 가사가 아니라 메타다.
  - **머리말은 떼지 않는다.** `300만의 목소리`처럼 가사 앞에 제목·연도 줄이 붙는 경우가 있지만,
    '첫 연에 제목과 같은 줄이 있으면 지운다'는 규칙은 **첫 소절이 곧 제목인 곡의 가사를 날린다**.
    드문 경우라 검수자가 지우는 게 안전하다(rules.md 12장 — 추측하지 않는다).
- **`source`(가사 출처)는 가사를 가져왔을 때만 붙인다**(PRD 9장, merge.py와 같은 규칙). 이름은
  호스트가 아니라 **채널명**(예: `아누즈`)이다 — "가사 출처: www.youtube.com"은 무의미하다.
  아누즈가 파랑검정을 credit해도 **우리가 실제로 가져온 곳은 그 영상**이라 출처는 아누즈다.
- **`type`은 `live` 기본값** — 아누즈는 팬 촬영 채널이다(merge.py와 같은 규칙). 공식 영상이면
  검수자가 `official`로 바꾼다.
- **제목 정리**: 아누즈 신곡 제목은 `제목; 인천유나이티드 응원가 (vs 상대)@경기장 YYMMDD` 형식이라
  `;` 앞만 취한다. 형식이 다르면 원제목을 그대로 두고 **검수자가 고친다** — 추측하지 않는다.
- **중복 제거**: 기존 곡(제목 키·`youtubeId`)과 이미 큐에 있는 후보를 모두 대조한다. 같은 URL을
  두 번 넣어도 후보가 늘지 않는다(멱등).

> **음성 추출(ASR)은 쓰지 않는다**: rules.md 9장이 "유튜브 영상은 임베드로만 불러온다 — 영상 파일을
> 직접 복사하지 않는다"고 못박고 있고, robots.txt도 `/get_video`를 Disallow한다. 대상이 관중 떼창
> 직캠이라 정확도도 낮고 출처도 못 밝힌다. 설명란이 모든 면에서 낫다.

### 3.9.4 현재 반영 상태 (2026-07-17)

아누즈 신곡 **9곡을 승격 완료** — `data/songs.json`이 34곡 → **43곡**(id 36~44: 이명주 콜·
김동민 콜·일기·Glory Incheon·Walk Along·300만의 목소리·가자 아시아로·인천 만세·동행).

> **가사 채움 완료** *(v2.7)*: 9곡 모두 영상 설명란에서 가사를 받아 `source`(출처: 아누즈)까지
> 붙었다 — `backfill_youtube_lyrics.py`로 처리(3.9.5). 목록·제목 검색뿐 아니라 **가사 검색(F-11)도
> 동작한다**. `300만의 목소리`는 설명란 머리말("2023 인천유나이티드 새 응원가" 2줄)이 섞여 들어와
> 손으로 제거했다.
>
> **분류 완료**: `이명주 콜`·`김동민 콜`은 `PLAYER_CHANTS`에 추가돼 선수 응원가로 분류됐다
> (이명주=현역, 김동민=미사용). 3.8.2 참고 — 새 선수 응원가는 `OOO 콜` 형식으로 거기 추가할 것.
>
> 현재 `songs.json`에서 **가사가 없는 곡은 `뱃고동소리(GOAL)` 하나**(효과음 트랙 — 공식 페이지도
> "가사가 없습니다"), **출처가 없는 곡은 0곡**이다.

### 3.9.5 backfill_youtube_lyrics.py — 이미 등록된 곡의 가사 채우기

`add_youtube_chants.py`는 **새 곡**을 큐에 넣는다. 반면 이미 `songs.json`에 올라간 곡은 큐를 다시
거치지 않고, `reconcile_songs.py`는 공식·커뮤니티 두 소스만 알아서 유튜브를 모른다. 그 틈을 메운다.

```bash
cd scraper
python backfill_youtube_lyrics.py --dry-run   # 무엇이 채워질지 미리 보기
python backfill_youtube_lyrics.py             # 가사 빈 곡 → 설명란에서 채움 (+source)
```

- **가사가 빈 곡만** 건드린다. 이미 가사가 있으면 건너뛴다 — 손으로 고친 가사나 공식 가사를 팬 채널
  설명란으로 덮어쓰지 않는다(3.7.2의 공식 > 커뮤니티 서열에서 설명란은 맨 아래다).
- 가사를 채우면 `source`도 함께 붙인다(PRD 9장). 설명란이 비었거나 못 읽으면 그대로 둔다.
- **멱등** — 두 번 실행해도 이미 채워진 곡은 건너뛴다.

## 3.10 상대 스카우팅(scouting) 크롤링

`export_scouting.py`가 K리그1 12팀의 **주요 선수·부상**을 모아 `data/opponent-scouting.json`
(팀명 키 → `{keyPlayers, injuries, probableLineup}`)으로 내보낸다. 화면은 '다가오는 상대'의 것을
골라 쓴다(`lib/api/opponentScouting.ts` → `OpponentScoutingCard`).

- **주요 선수**: 다음 선수 순위 API(`client.fetch_person_rank`)에서 팀별 **득점→도움 순 상위 2명**.
  다른 구단 데이터와 같은 다음 스포츠 소스라 팀명 정규화도 `codes.py`를 공유한다.
- **부상**: Transfermarkt K리그1 부상자 목록을 BeautifulSoup으로 파싱(선수·부상명·복귀 예정).
  다음이 부상 정보를 주지 않아 유일하게 **다른 소스**다 — 팀명은 `codes.normalize_tm_team_name()`
  (영문→앱 표기)으로 맞춘다. 페이지 구조 변경 등으로 실패하면 **주요 선수만이라도** 나가도록
  부상은 빈 값으로 건너뛴다.
- **라인업(probableLineup)**: 신뢰할 소스가 없어 **빈 목록**으로 둔다(화면도 비워 처리).
- **meta 없음**: 스카우팅은 매 실행 전체를 덮어쓰고, 화면의 "○○ 기준" 배지는 상대 폼 meta를
  재사용한다(같은 크론에서 함께 갱신되므로) — 그래서 별도 `*-meta.json`을 만들지 않는다.

> **F-18 정책 메모**: 이 문서는 원래 "주요 선수·부상은 개인정보 리스크로 mock 유지"라고 적었으나,
> 구현은 이미 위 실데이터를 수집·표시하고 있었다(앱 `opponentScouting.ts`도 "실데이터"로 주석).
> 2026-07-19 문서를 코드에 맞췄다. 공개된 스탯·부상 정보만 다루고 선수 사생활은 담지 않는다.

## 4. 이력 보존 (수집일별 스냅샷)

순위표는 수집일별 갱신 스냅샷이므로, 최신만 덮어쓰지 않고 **수집일별로 누적 보존**한다.
...
`.github/workflows/update-kleague-data.yml` — **매일 06:00 KST(= 21:00 UTC)** 크론.
다섯 크롤러 실행(순위표·경기·상대 폼·상대전적·스카우팅) → 변경된 `data/*.json` 커밋·푸시.
`workflow_dispatch`로 수동 실행도 가능. **전제**: 레포가 GitHub에 있어야 동작한다(미연결이면
`git init`·푸시 후 활성화).

> **주 2회 → 매일 전환(2026-07-19)**: 경기 다음 날 아침에 결과가 바로 반영되게 매일로 바꿨다.
> - **비용**: private 레포 기준 1회 ≈ 2~3분 × 30일 ≈ 90분/월로 무료 한도(2,000분) 안이다.
> - **순위 변동(▲▼)**: `previous_snapshot()`이 '어제' 스냅샷과 비교하게 되므로, 경기 없는 날은
>   전 팀이 '유지(–)'로 표시된다(4.1절). 틀린 값은 아니고 비교 창이 좁아질 뿐이다.
> - **재배포**: 수집일별 이력 파일이 매 실행 새로 생겨(4절) 순위 무변동일에도 커밋·재배포가
>   일어난다 — 매일 1회 Vercel 재배포. 이력 파일은 연 ~365개(작은 JSON)로 늘어난다.
...
- **상대 라인업(probableLineup)**: 신뢰할 소스가 없어 빈 목록 유지 중. 주요 선수·부상은 실데이터로
  수집한다(3.10절).
...
| 버전 | 날짜 | 내용 |
|------|------|------|
| v2.8 | 2026-07-19 | **구단 데이터 크론을 주 2회(월·목) → 매일 06:00 KST로 전환**(5절). 경기 다음 날 아침에 결과가 반영되도록. 비용(≈90분/월)·순위 변동 배지 의미(어제 대비)·매일 재배포 트레이드오프를 5절에 명시. 워크플로우 이름·주석·커밋 메시지를 '주간'→'일간'으로 갱신. **스카우팅 문서를 코드에 맞춤**(3.10절 신설) — 3.5·7절이 "주요 선수·부상은 mock 유지"라 적었으나 `export_scouting.py`·앱은 이미 실데이터(다음 선수 순위·Transfermarkt 부상)를 수집·표시 중이었다. 문서화 안 됐던 `export_scouting.py`를 2절 파일 구조에 추가하고, 다른 크롤러와 스타일 일관화(한국어 주석·pathlib·다음 접근을 `client.fetch_person_rank`로 통일) |
| v2.7 | 2026-07-17 | **아누즈 가사를 영상 설명란에서 수집**(3.9.3) — 채널이 설명란에 가사 전문을 적어두고 출처(파랑검정)까지 밝혀둔다. `출처 :` 꼬리표만 떼고 머리말은 검수자에게 맡긴다(첫 소절이 제목인 곡의 가사를 날릴 위험). `backfill_youtube_lyrics.py` 추가(3.9.5)로 이미 승격된 9곡의 가사·출처를 채움 — 가사 검색(F-11)까지 동작. **ASR(음성 추출)은 rules.md 9장·robots.txt 위반이라 탈락**. 3.7.1 소스 표에서 아누즈를 '부분 채택'→'채택'으로 승격 |
| v2.6 | 2026-07-17 | **선수 응원가 제목을 `OOO 콜`로 통일**(3.8.2) — 이명주 콜·김동민 콜 추가로 8곡. 크롤 소스는 맨 이름을 쓰므로 `normalize_title`이 '콜' 접미사를 흡수하게 함(안 그러면 가사 갱신 중단·중복 추가). **`reconcile_songs.py` 삭제 규칙 정정**(3.8.1) — "출처 없으면 삭제"가 가사 없는 곡까지 지워, 승격한 아누즈 9곡을 전부 삭제할 뻔했다. 아누즈 신곡 9곡 승격 — songs.json 34곡 → 43곡(3.9.4). **아누즈 채널 신곡 반영 경로 추가**(3.9절) — 지금 영상 대부분이 이미 아누즈 것이지만 인천네이션 게시글(2016~2019년 링크 모음)을 거쳐 들어와 이후 신곡이 누락돼 있었다. 채널 RSS(`/feeds/videos.xml`)와 `/youtubei/`가 **robots.txt Disallow**라 자동 수집은 채택하지 않고(나무위키 탈락 기준과 동일), 허용된 `oembed`로 **사람이 고른 URL만** 조회해 검수 큐에 넣는다. YouTube Data API는 API 키 의존 부활이라 탈락 |
