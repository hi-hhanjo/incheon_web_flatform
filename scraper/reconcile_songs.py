...
2. **가사의 출처를 못 밝히면 지운다** — 1을 적용하고도 `source`가 없는 곡은 삭제한다.
   PRD 9장(가사는 출처를 표기하거나 공식/공개 자료를 사용)을 만족할 수 없는 데이터다.
   단 **가사가 빈 곡은 남긴다** — 밝힐 가사가 없으니 출처 없음이 결함이 아니다(영상만 있는
   아누즈 신곡이 여기 해당, CRAWLER_SPEC.md 3.9절).
...
def main() -> None:
    ...
    for song in songs:
        ...
        # 가사가 **있는데** 출처를 못 밝히는 곡만 지운다(방침 2). 가사가 빈 곡(영상만 있는 곡 —
        # 3.9절 아누즈 신곡)은 밝힐 가사가 없으니 '출처 없음'이 결함이 아니다. 조건에서 lyrics를
        # 빼면 그런 곡이 통째로 삭제된다 — 실제로 그럴 뻔했다(CRAWLER_SPEC.md 3.8.1 v2.6 정정).
        if song["lyrics"] and "source" not in song:
